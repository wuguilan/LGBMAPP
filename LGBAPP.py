...
if os.path.exists('requirements.txt'):
    st.success("✅ requirements.txt 文件存在")
    with open('requirements.txt', 'r', encoding='utf-8') as f:
        content = f.read()
        st.write("requirements.txt 内容:")
        st.code(content)
else:
...
st.text(installed_packages[:500])  # 只显示前500个字符

# 尝试安装 joblib
st.write("尝试安装 joblib...")
try:
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", "joblib==1.3.2"], 
        capture_output=True, 
        text=True,
        check=True
    )
    st.success("✅ joblib 安装成功")
    st.code(result.stdout)
except subprocess.CalledProcessError as e:
    st.error(f"❌ joblib 安装失败: {e}")
    st.code(e.stdout)
    st.code(e.stderr)
...
try:
    ...
    st.error(f"❌ 第二次导入仍然失败: {e}")
    
    # 尝试使用不同的安装方法
    st.write("尝试使用 --user 标志安装...")
    subprocess.run([sys.executable, "-m", "pip", "install", "--user", "joblib==1.3.2"])
